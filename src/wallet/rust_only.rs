...
use crate::utils::{
    RgbRuntime, RgbRuntimeLock, acquire_rgb_runtime_lock, hash_bytes_hex,
    load_rgb_runtime_for_operation,
};
#[cfg(any(feature = "electrum", feature = "esplora"))]
...
impl PreparedRgbTransferAcceptance {
    ...
    /// Hex-encoded digests of media attachments declared by the validated transfer.
    pub fn media_digests(&self) -> &HashSet<String> {
        &self.media_digests
    }
...
impl Wallet {
    ...
    fn color_psbt_with_runtime(
        ...
        for (contract_id, asset_coloring_info) in coloring_info.asset_info_map.clone() {
            let schema = AssetSchema::get_from_contract_id(contract_id, runtime).map_err(|_| {
                Error::AssetNotFound {
    ...
    /// Color and consume a PSBT as the owner of a promoted protocol operation.
    ///
    /// Normal wallet access remains blocked while a transactional acceptance is pending. This
    /// method grants access only when `operation_id` matches the promoted journal owner, keeping
    /// commitment construction inside the same rollback boundary as the surrounding protocol.
    #[cfg(any(feature = "electrum", feature = "esplora"))]
    pub fn color_psbt_and_consume_for_operation(
        ...
        runtime.consume_fascia(fascia.clone(), witness_ord)?;
        Ok(fascia)
    }
    ...
    pub fn accept_transfer_consignment(
        ...
        let valid_consignment = match consignment
            .clone()
            .validate(&fallback_resolver, &validation_config)
        {
        ...
        let media_digests = self
            ...
            .collect::<HashSet<_>>();
        runtime.import_contract(valid_contract, &fallback_resolver)?;

        let received_rgb_assignments =
            self.extract_received_assignments(&consignment, witness_id, Some(vout), None);

        runtime.accept_transfer(valid_consignment, &fallback_resolver)?;
    ...
    fn prepare_accept_transfer_consignment(
        ...
        let staged_stock =
            runtime.stage_transfer(graph_seal, valid_consignment, &fallback_resolver)?;
    ...
    /// Manually set the [`WitnessOrd`] of a witness TX while owning a matching promoted protocol
    /// operation.
    ///
    /// Normal RGB runtime access remains blocked while a transactional acceptance is pending. This
    /// method grants access only when `operation_id` matches the promoted journal owner, keeping
    /// the witness mutation inside the same rollback boundary as the surrounding protocol.
    ///
    ...
    #[cfg(any(feature = "electrum", feature = "esplora"))]
    pub fn upsert_witness_for_operation(
        &self,
        operation_id: &str,
        witness_id: RgbTxid,
        witness_ord: WitnessOrd,
    ...
    pub fn save_new_asset(
        ...
        let valid_transfer = match consignment
            .clone()
            .validate(&fallback_resolver, &validation_config)
        {
            Ok(consignment) => consignment,
    ...
    pub fn send_end_db_update_only(
        ...
        self.send_end_db_update_only_impl(online, signed_psbt, None)
    }

    /// Complete the donation send operation by updating the DB only while owning a matching
    /// durable RGB stock operation.
    ///
    /// The signed transaction ID must match `operation_id`. Normal RGB runtime access remains
    /// blocked while the operation is promoted; this entry point grants access only to the exact
    /// journal owner and retains the stock lock through broadcast and database commit.
    ///
    /// <div class="warning">This method is intended only for protocol integrations that provide
    /// their own durable commit decision and exchange consignments out of band.</div>
    #[cfg(any(feature = "electrum", feature = "esplora"))]
    pub fn send_end_db_update_only_for_operation(
        &mut self,
        online: Online,
        operation_id: &str,
        signed_psbt: String,
        ...
        self.send_end_db_update_only_impl(online, signed_psbt, Some(operation_id))
    }

    #[cfg(any(feature = "electrum", feature = "esplora"))]
    fn send_end_db_update_only_impl(
        &mut self,
        online: Online,
        signed_psbt: String,
        operation_id: Option<&str>,
        ...
        if operation_id.is_some_and(|operation_id| operation_id != txid) {
            return Err(Error::Internal {
                details: s!("RGB protocol operation ID does not match the send transaction"),
            });
        }
        let _runtime_guard = match operation_id {
            Some(operation_id) => load_rgb_runtime_for_operation(self.wallet_dir(), operation_id)?,
            None => self.rgb_runtime()?,
        };
        let txn = self.database().begin_transaction()?;
        ...
        let (transfer_txid, _, info_contents, _) = self.get_transfer_end_data(&psbt)?;
        debug_assert_eq!(transfer_txid, txid);
