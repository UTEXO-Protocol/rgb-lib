...
pub(super) fn psbt_with_foreign_tap_script_sigs(signed_psbt: &str, foreign_signer_count: usize) -> String {
    ...
    for input in psbt.inputs.iter_mut() {
        let leaves: Vec<TapLeafHash> = input
            .tap_script_sigs
            .keys()
            .map(|(_, leaf)| *leaf)
            .collect();
        assert!(
            !leaves.is_empty(),
            "cosigner-signed PSBT must expose tap_script_sigs before adding foreign signatures"
        );
        for i in 0..foreign_signer_count {
            let secret_key = SecretKey::from_slice(&[((i + 1) * 41) as u8; 32]).unwrap();
            let keypair = Keypair::from_secret_key(&secp, &secret_key);
            let (xonly, _) = XOnlyPublicKey::from_keypair(&keypair);
            let fake_sig = secp256k1::schnorr::Signature::from_slice(&[0xAB; 64]).unwrap();
            let tap_sig = TapSignature {
                signature: fake_sig,
                sighash_type: TapSighashType::Default,
            };
            for leaf in &leaves {
                input.tap_script_sigs.insert((xonly, *leaf), tap_sig);
            }
        }
    }
    psbt.to_string()
